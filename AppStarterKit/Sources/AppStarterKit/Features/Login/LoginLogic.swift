...
/// and is what `AuthServicing` (never a DTO, M2) hands back.
...
/// `AuthServicing`, persisting the session — nothing else in the app calls
/// `AuthServicing.login` or writes to `SessionStoring` on a successful login.
///
