...
final class SettingsUITests: AppStarterUITestCase {
    func testBrandThemeAppliesLiveAndPinningTogglesUpdateSummary() {
        ...
        // One helper per block, in the same order and with the same assertions: the single
        // function went past SwiftLint's `function_body_length` under `--strict`.
        turnBrandThemeOn(themeToggle, in: app)
        runDiagnosticsUnderBrandTheme(in: app)
        turnBrandThemeOff(themeToggle, in: app)
        togglePinningAndCheckSummary(in: app)
    }
    ...
    private func turnBrandThemeOn(_ themeToggle: XCUIElement, in app: XCUIApplication) {
    ...
    private func runDiagnosticsUnderBrandTheme(in app: XCUIApplication) {
    ...
    private func turnBrandThemeOff(_ themeToggle: XCUIElement, in app: XCUIApplication) {
    ...
    private func togglePinningAndCheckSummary(in app: XCUIApplication) {
