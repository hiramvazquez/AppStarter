...
/// `refreshToken` pair, plus the id of the signed-in user (used by `ProfileFeature` to
...
/// `BearerTokenInterceptor` and written by `LoginFeature`/the refresh retrier in
/// `Networking`, exactly like `AppFoundation/Examples/LoginApp`'s `SessionStoring`).
/// Pure Foundation — no CoreNetworking, no AppFoundation — so it lives in `Domain`, not in
/// `Networking`: any feature can depend on it directly without pulling in networking.
///
