...
/// see `NetworkingModule`) and replays the request once on a 401. When the refresh itself
/// fails, the session cannot recover: `sessionStore` is invalidated and `sessionExpiring`
/// is notified, so `RootView` routes back to `.login` with a banner (M6, mirrors
/// `AppFoundation/Examples/LoginApp/Sources/LoginApp/Features/Login/Services/LoginService.swift`).
///
/// This wiring lives in `Networking` (`Packages/Platform`), not inside any one feature's
/// `Module` and not in `App/` (PRD-APP-02, Fase 1: "el wiring del APIService autenticado
/// va a un target Networking... que importa CoreNetworking y Domain") — it is genuinely
/// cross-feature: every authenticated request in the app goes through the ONE
