...
/// `SessionStoring`, both already registered by `Networking`'s `NetworkingModule`) and
/// `LoginViewModel`. `NetworkingModule` must be registered before this one (see
/// `App/AppModule.swift`'s module list) — laziness means the ORDER of
/// `register(modules:)` doesn't matter for correctness, only that every module is
