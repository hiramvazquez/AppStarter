...
/// — the ONE place it is constructed — and registers `FavoritesStoring` (`Domain`) from
/// it, which `ProductDetailModule` resolves too (shared store, two features).
public struct FavoritesModule: DependencyModule {
    ...
    /// the architecture's Store boundary), so a caller outside `FavoritesFeature` (the
    /// app target's `-UITestOffline` wiring, in `App/`) can't build this `ModelContainer`
    /// itself — this is the seam that lets it ask for one anyway.
